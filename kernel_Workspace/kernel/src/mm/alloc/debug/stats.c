...
void alloc_stats_note_alloc(size_t size) {
	...
	stat_total_allocs++;
	if (stat_live_bytes > stat_peak_bytes) stat_peak_bytes = stat_live_bytes;
	smp_lock_release(&stats_smp_lock);
}
void alloc_stats_note_free(size_t size) {
	smp_lock_acquire(&stats_smp_lock);
	if (stat_live_count > 0) stat_live_count--;
	if (stat_live_bytes >= size) stat_live_bytes -= size;
	else stat_live_bytes = 0;
	stat_total_frees++;
	smp_lock_release(&stats_smp_lock);
}
...
void alloc_stats_dump(void) {
	smp_lock_acquire(&stats_smp_lock);
	size_t
		live_count = stat_live_count,
		live_bytes = stat_live_bytes,
		peak_bytes = stat_peak_bytes,
		total_allocs = stat_total_allocs,
		total_frees = stat_total_frees;
	smp_lock_release(&stats_smp_lock);
	kprintf("ALLOC STATS:\n");

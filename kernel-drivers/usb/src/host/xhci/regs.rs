...
impl XhciRegs {
    pub fn new(phys: u64) -> Result<Self, UsbError> {
        let ptr = if phys >= 0xFFFF800000000000 {
            ...
            if !unsafe { hw_sys::mmio_map(phys, MAP_SIZE, &mut virt) } {
                return Err(UsbError::MapFailed);
            }

            // The runtime can report success yet leave the MMIO unmapped;
            // reading it would page-fault, so verify.
            if !unsafe { hw_sys::paging_is_mapped(virt) } {
                return Err(UsbError::MapFailed);
            }

...
static uint64_t *dbg_tail_ptr(void *user, size_t size) { return (uint64_t *)((uint8_t *)user + size); }

void *dbg_alloc(size_t size) {
	if (size == 0) return NULL;

	uint64_t caller = (uint64_t)(uintptr_t)__builtin_return_address(0);
	uint8_t *real = (uint8_t *)heap_alloc(sizeof(dbg_header_t) + size + sizeof(uint64_t));
...
void dbg_free(void *ptr) {
	...
	dbg_header_t *h = (dbg_header_t *)((uint8_t *)ptr - sizeof(dbg_header_t));
	...
	for (size_t i = 0; i < total; i++) real[i] = DBG_POISON_FREE;
...
bool dbg_verify(void *ptr) {
	...
	dbg_header_t *h = (dbg_header_t *)((uint8_t *)ptr - sizeof(dbg_header_t));
	if (h->magic != DBG_MAGIC) return false;

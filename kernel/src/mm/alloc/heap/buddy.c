...
static size_t buddy_pages = 0;
static size_t buddy_max_order = 0;

static uint8_t *buddy_order = NULL;
static uint8_t *buddy_state = NULL;
static int32_t *buddy_next = NULL;
static int32_t *buddy_heads = NULL;
...
static size_t buddy_used_bytes = 0;
static size_t buddy_free_bytes = 0;

static inline uint64_t order_size(size_t o)
{
    return ARCH_PAGE_SIZE << o;
}

static size_t page_of(uint64_t va)
{
    return (size_t)((va - buddy_base) / ARCH_PAGE_SIZE);
}

static uint64_t va_of(size_t page)
{
    return buddy_base + (uint64_t)page * ARCH_PAGE_SIZE;
}

static size_t size_to_order(size_t size)
{
    size_t o = 0;
    size_t s = ARCH_PAGE_SIZE;
                ...
                buddy_unmap_cb unmap_cb)
{
    ...
    while (order_size(buddy_max_order) < size) {
        buddy_max_order++;
    }

    size_t off_order = 0;
    size_t off_state = buddy_pages;
    size_t off_next = (off_state + buddy_pages + 7) & ~(size_t)7;
    size_t off_heads = (off_next + buddy_pages * 4 + 7) & ~(size_t)7;
    size_t meta_bytes = off_heads + (buddy_max_order + 1) * 4;

    uint64_t meta_phys = 0;

    if (!pmm_alloc_bytes(meta_bytes, &meta_phys)) {
        return false;
    }
    ...
    for (size_t o = 0; o <= buddy_max_order; o++) {
        buddy_heads[o] = -1;
    }
...
void *buddy_alloc(size_t size)
{
    ...
    if (order_size(o) < size || o > buddy_max_order) {
        return NULL;
    }

    size_t cur = o;

    while (buddy_heads[cur] < 0 && cur < buddy_max_order) {
        cur++;
    }

    if (buddy_heads[cur] < 0) {
        return NULL;
    }
    ...
    while (cur > o) {
        ...
        size_t half_pages = (size_t)1 << cur;
        size_t buddy_idx = idx + half_pages;
...
{
    if (!buddy_initialized || ptr == NULL) {
        return;
    }

    uint64_t va = (uint64_t)(uintptr_t)ptr;

    if (va < buddy_base || va >= buddy_base + (uint64_t)buddy_pages * ARCH_PAGE_SIZE) {
        return;
    }

    size_t idx = page_of(va);

    if (buddy_state[idx] != BUDDY_STATE_USED) {
        return;
    }

    size_t o = buddy_order[idx];
    size_t bytes = order_size(o);
...
{
    return buddy_used_bytes;
}

size_t buddy_stat_free_bytes(void)
{
    return buddy_free_bytes;
}

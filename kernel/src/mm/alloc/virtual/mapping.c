...
static bool range_valid(uint64_t virt, uint64_t phys, size_t len)
{
    if (len == 0) {
        return true;
    }

    if (!arch_is_page_aligned(virt) ||
        !arch_is_page_aligned(phys) ||
        !arch_is_page_aligned((uint64_t)len)) {
        return false;
    }

    if ((uint64_t)len > UINT64_MAX - virt) {
        return false;
    }

    if ((uint64_t)len > UINT64_MAX - phys) {
        return false;
    }
                         ...
                         size_t len)
{
    ...
    if (!arch_is_page_aligned(virt) || !arch_is_page_aligned((uint64_t)len)) {
        return false;
    }
                        ...
                        size_t len)
{
    ...
    if (!arch_is_page_aligned(dst_virt) ||
        !arch_is_page_aligned(src_virt) ||
        !arch_is_page_aligned((uint64_t)len)) {
        return false;
    }
    ...
    for (size_t i = 0; i < pages; i++) {
        ...
        if (dp == UINT64_MAX || sp == UINT64_MAX) {
            return false;
        }

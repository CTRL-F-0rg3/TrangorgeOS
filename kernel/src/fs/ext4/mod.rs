...
//static mut EXT4_FS: Option<Ext4> = None;

// pub fn fs_mount_all() {
//     let n = fs::driver::registry::count();

//     for i in 0..n {
//         if let Some(d) = fs::driver::registry::get(i) {
//             if let Ok(fs) = Ext4::mount(d) {
//                 unsafe { EXT4_FS = Some(fs) };
//                 break;
//             }
//         }
//     }
...
//     let path = if len == 0 { "/" } else { /* z arg/len */ "/" };

//     if let Some(fs) = unsafe { &EXT4_FS } {
//         if let Ok(entries) = fs.list_path(path) {
//             for e in entries {
//                 let kind = if e.ftype == 2 { "d" } else { "-" };
//                 kprintf(b"%s %s\n\0", kind, e.name.as_ptr());
//             }
//         }
//     }
...
// extern "C" fn cmd_cat(arg: *mut u8, len: u32) -> i32 {
//     if let Some(fs) = unsafe { &EXT4_FS } {
//         let mut buf = vec![0u8; 4096];

//         if let Ok(n) = fs.read_path("/readme.txt", &mut buf) {
//             for i in 0..n {
//                 kprintf_char(buf[i]);
//             }
//         }
...
// nimcore::shell_register("ls", cmd_ls);
// nimcore::shell_register("cat", cmd_cat);